...
@router.post("/create-folder", dependencies=[Depends(require_recruiter)])
async def create_folder(folder_name: str = Form(...)):
    result = file_manager.create_folder(folder_name)
    return result

@router.get("/folders", dependencies=[Depends(require_recruiter)])
async def get_folders():
    folders = file_manager.get_folders()
    return {"folders": folders}

@router.get("/list", dependencies=[Depends(require_recruiter)])
async def list_cvs():
